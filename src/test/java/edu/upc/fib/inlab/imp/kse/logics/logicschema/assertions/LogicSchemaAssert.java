...
public class LogicSchemaAssert extends AbstractAssert<LogicSchemaAssert, LogicSchema> {
    ...
    /**
     * Asserts whether the actual logicSchema is equivalent to the expectedSchema. That is, whether they have equivalent
     * predicates (base, and derived), and equivalent constraints. Do note that this comparison is NOT agnostic with the
     * name of the derived predicates.
     * <p>
     * Since this check is not decidable, this method applies a sound (but not complete) strategy. In particular, for
     * each normal clause of one schema, it tries to find a homomorphic normal clause of the other schema, and
     * viceversa.
     *
     ...
     * <p>
     * Since this check is not decidable, this method applies a sound (but not complete) strategy. In particular, for
     * each derivation rule of one schema, it tries to find a homomorphic derivation rule of the other schema, and
     * viceversa.
     *
     * @param expectedSchema not null
     * @return this assert
    ...
    /**
     * Asserts whether the actual logicSchema constraints are equivalent to the expectedSchema constraints. Do note that
     * this comparison IS agnostic with the name of the derived predicates.
     * <p>
     * Since this check is not decidable, this method applies a sound (but not complete) strategy. In particular, for
     * each logicConstraint of one schema, it tries to find a homomorphic logicConstraint of the other schema, and
     * viceversa. In such case, two derived ordinary literals are considered to be homomorphic if they have homomorphic
     * definition rules. In practice, this means that two logic constraints are equivalent if they are the same up-to
     * renaming of variables and derived predicate names.
     *
     * @param expectedSchema not null
     * @return this assert
     */
    ...
    /**
     * Checks whether the actual schema contains a constraint equivalent to expectedConstraint considering that two
     * derived ordinary literals are equivalent if their derivation rules are equivalent
     *
