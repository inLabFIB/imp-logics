...
public class LogicSchemaAugmenter extends LogicSchemaProcess {
    ...
    private void createEventPredicates() {
        for (Predicate p : inputLogicSchema.getAllPredicates()) {
            (new EventPredicate(p, EventPredicate.EventType.INSERT)).copyToLogicSchema(augmentedSchema);
            (new EventPredicate(p, EventPredicate.EventType.DELETE)).copyToLogicSchema(augmentedSchema);
        }
    }
    ...
    private void addInsDerivationRule(Predicate p, Atom pHead, List<List<Literal>> augmentedPLiterals) {
        Predicate ins_p = augmentedSchema.getPredicate(EventPredicate.getPredicateNameFor(EventPredicate.EventType.INSERT, p));
        for (List<Literal> pPostBody : augmentedPLiterals) {
    ...
    private void addDelDerivationRule(Predicate p, Atom pHead, List<Literal> pLiterals, Atom pPostHead) {
        Predicate del_p = augmentedSchema.getPredicate(EventPredicate.getPredicateNameFor(EventPredicate.EventType.DELETE, p));
    ...
    private OrdinaryLiteral getInsertion(OrdinaryLiteral oliteral) {
        return getEvent(EventPredicate.EventType.INSERT, oliteral);
    }
    ...
    private OrdinaryLiteral getDeletion(OrdinaryLiteral oliteral) {
        return getEvent(EventPredicate.EventType.DELETE, oliteral);
    }

    /**
     * @return a copy of the given literal changing its predicate to its corresponding event predicate
     */
    private OrdinaryLiteral getEvent(EventPredicate.EventType eventType, OrdinaryLiteral oliteral) {
