 ...
 * list of derivarion rules of olit. If olit is base, or it has only one derivation rule it returns olit.
 */
    protected List<Literal> getMultipleDerivationRuleUnfolding(OrdinaryLiteral olit) {
        List<Literal> result = new LinkedList();
        
        if(olit.isBase() || olit.getDefinitionRulesWhenCalled(new LinkedList<Literal>()).size() == 1){
            result.add(olit);
            return result;
        }
        else {
            int n = 1;
            for(List<Literal> literals: (List<List<Literal>>)olit.getDefinitionRulesWhenCalled(olit.getVariablesNames())){
            ...
            if(lit instanceof BuiltInLiteral){
                ...
                if(olit.isNegated()){
                    ...
                    if(olit.isBase()){
                        ...
                        List<Literal> restOfLiterals = new LinkedList(currentLiterals);
                        restOfLiterals.remove(olit);
                        for(List<Literal> unfoldedLiterals: (List<List<Literal>>)olit.getDefinitionRulesWhenCalled(restOfLiterals)){
